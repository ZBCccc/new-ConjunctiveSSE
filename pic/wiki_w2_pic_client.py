...
# 预设文件地址（实际使用时替换）
file_paths = {
    "FDXT": "result/Search/FDXT/Wiki_USENIX/processed_w2_keywords_2.csv",
    "ODXT": "result/Search/ODXT/Wiki_USENIX/processed_w2_keywords_2.csv",
    "SDSSE-CQ": "result/Search/SDSSE-CQ/Wiki_USENIX/processed_w2_keywords_2.csv"
}
...
for scheme, file_path in file_paths.items():
    try:
        ...
        if scheme == "SDSSE-CQ":
            y_values = (df['clientTime'] + 0.4 * 9738) / 1000.0
        elif scheme == "FDXT":
            y_values = (df['clientTime'] + 1250) / 1000.0
...
y_ticks = [45, 90, 135, 180]
y_labels = ['45', '90', '135', '180']
ax_main.set_yticks(y_ticks)
ax_main.set_yticklabels(y_labels, fontsize=50)
ax_main.set_ylim(0, 180)  # 略微扩展范围使图表美观
...
# 设置线性刻度下的次要刻度线
ax_main.yaxis.set_minor_locator(plt.MultipleLocator(22.5))  # 每10个单位一个次要刻度
...
# 保存文件
plt.savefig("pic/w2-client/Wiki_client_computation_time.pdf", dpi=3600, format='pdf')
