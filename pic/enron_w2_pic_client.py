...
for scheme, file_path in file_paths.items():
    try:
        ...
        if scheme == "SDSSE-CQ":
            y_values = (df['clientTime'] + 0.4 * 26946) / 1000.0
        elif scheme == "FDXT":
            y_values = (df['clientTime'] + 2676) / 1000.0
...
ax_main.set_ylim(0, 500)  # 略微扩展范围使图表美观
...
# 设置线性刻度下的次要刻度线
ax_main.yaxis.set_minor_locator(plt.MultipleLocator(50))  # 每10个单位一个次要刻度
...
# 保存文件
plt.savefig("pic/w2-client/Enron_client_computation_time.pdf", dpi=3600, format='pdf')
