...
for scheme, file_path in file_paths.items():
    try:
        ...
        if scheme == "SDSSE-CQ":
            y_values = (fixed_times[scheme] + random_fluctuations + 3 * (x_values - 1)) / 1000.0
...
# 设置插图y轴刻度
inset_y_ticks = np.arange(1, 5, 1)  # 根据您的范围调整
...
ax_inset.yaxis.set_minor_locator(plt.MultipleLocator(0.5))
# 为插图添加y轴标签，明确单位
ax_inset.set_ylabel('ms', fontsize=30)

# 可以选择添加矩形框来标识主图中放大的区域
# 从主图中的(0, 0)到(3500, 0.07)画一个虚线矩形
...
# 保存文件
plt.savefig("pic/w1-server/Crime_server_computation_time_comparison.pdf", dpi=3600, format='pdf')
