...
# 预设文件地址
file_paths = {
    "Nomos": "pic/nomos_client_data/Nomos_Enron.csv",
    "MC-ODXT": "pic/nomos_client_data/MC-ODXT_Enron.csv",
    "VQNomos": "pic/nomos_client_data/VQNomos_Enron.csv"
}
...
for scheme, file_path in file_paths.items():
    try:
        ...
        # 分别处理小于4000和大于等于4000的部分
        x_values_below_4000 = x_values[x_values < 4000]
        y_values_below_4000 = y_values[x_values < 4000]

        x_values_above_4000 = x_values[x_values >= 4000]
        y_values_above_4000 = y_values[x_values >= 4000]

        # 选择每隔500个点
        indices_below_4000 = np.arange(500, len(x_values_below_4000), 500)
        x_sampled_below_4000 = x_values_below_4000.iloc[indices_below_4000]
        y_sampled_below_4000 = y_values_below_4000.iloc[indices_below_4000]

        # 选择每隔1000个点
        indices_above_4000 = np.arange(1000, len(x_values_above_4000), 1000)
        x_sampled_above_4000 = x_values_above_4000.iloc[indices_above_4000]
        y_sampled_above_4000 = y_values_above_4000.iloc[indices_above_4000]

        # 最后一个点被包含
        last_point_above = x_values_above_4000.iloc[[-1]]

        # 合并采样结果，确保最后一个点被包含
        x_sampled = pd.concat([x_sampled_below_4000, x_sampled_above_4000, last_point_above])
        y_sampled = pd.concat([y_sampled_below_4000, y_sampled_above_4000, y_values_above_4000.iloc[[-1]]])
...
ax_main.yaxis.set_minor_locator(FixedLocator([25, 75, 125, 175]))
...
# 保存文件
plt.savefig("pic/nomos_client_storage/Enron_client_storage_comparison.pdf", dpi=3600, format='pdf')
