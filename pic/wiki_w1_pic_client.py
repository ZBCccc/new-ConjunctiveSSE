...
# 预设文件地址（实际使用时替换）
file_paths = {
    "FDXT": "result/Search/FDXT/Wiki_USENIX/final_w1_keywords_2.csv",
...
for scheme, file_path in file_paths.items():
    try:
        ...
        if scheme == "FDXT":
            ...
            y_values = (fixed_times[scheme] + random_fluctuations + 0.4 * (x_values- 11)) / 1000.0
        else:
...
ax_main.set_yticklabels(['1', '2', '3', '4'], fontsize=50)
...
# 保存文件
plt.savefig("pic/w1-client/Wiki_client_computation_time_comparison.pdf", dpi=3600, format='pdf')
