...
# 预设文件地址（实际使用时替换）
file_paths = {
    "FDXT": "result/Search/FDXT/Enron_USENIX/final_w1_keywords_2.csv",
    "ODXT": "result/Search/ODXT/Enron_USENIX/w1_keywords_2.csv",
    "SDSSE-CQ": "result/Search/SDSSE-CQ/Enron_USENIX/w1_keywords_2.csv"
}
...
gs = gridspec.GridSpec(2, 1, height_ratios=[1, 8], hspace=0.02)

# 创建顶部的子图用于放置图例
ax_legend = plt.subplot(gs[0])
ax_legend.axis('off')  # 关闭坐标轴显示
# 设置图例子图的范围与主图一致
ax_legend.set_xlim(0, 30000)
...
ax_main.set_yticks([2, 4, 6, 8, 10, 12])
ax_main.set_yticklabels(['2', '4', '6', '8', '10', '12'], fontsize=50)
...
ax_main.yaxis.set_minor_locator(plt.MultipleLocator(1))  # 调整次要刻度间隔
...
# 保存文件
plt.savefig("pic/w1-client/Enron_client_computation_time_comparison.pdf", dpi=3600, format='pdf')
