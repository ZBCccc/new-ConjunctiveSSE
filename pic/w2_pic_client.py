...
# 设置主图属性
ax_main.set_xlabel('Keyword Volume', fontsize=42, fontweight='bold', labelpad=1)
ax_main.set_ylabel('Client Computation Time(ms)', fontsize=42, fontweight='bold', labelpad=1, rotation=90)

# 设置主图的x轴范围和刻度
ax_main.set_xlim(0, 30000)
x_ticks = [0, 5000, 10000, 15000, 20000, 25000, 30000]
ax_main.set_xticks(x_ticks)
ax_main.set_xticklabels([f'{x}' for x in x_ticks], fontsize=30)

# 使用线性刻度以清晰展示数据点间的线性关系
# plt.yscale('log')

# 设置适合数据范围(0.075-160ms)的y轴刻度
y_ticks = [0, 50, 100, 150, 200, 250, 300, 350, 400, 450]
y_labels = ['0', '50', '100', '150', '200', '250', '300', '350', '400', '450']
ax_main.set_yticks(y_ticks)
ax_main.set_yticklabels(y_labels, fontsize=30)
ax_main.set_ylim(0, 450)  # 略微扩展范围使图表美观
...
# 设置线性刻度下的次要刻度线
ax_main.yaxis.set_minor_locator(plt.MultipleLocator(10))  # 每10个单位一个次要刻度
...
# 保存文件
plt.savefig("pic/w2-client/Enron_client_computation_time.pdf", dpi=3600, bbox_inches='tight', format='pdf')
plt.savefig("pic/w2-client/Enron_client_computation_time.svg", dpi=3600, bbox_inches='tight', format='svg')
plt.savefig("pic/w2-client/Enron_client_computation_time.png", dpi=3600, bbox_inches='tight', format='png')
