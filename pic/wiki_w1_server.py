...
# 预设文件地址（实际使用时替换）
file_paths = {
    "FDXT": "result/Search/FDXT/Wiki_USENIX/2025-03-30_13-46-24w2_keywords_2.csv",
...
fig = plt.figure(figsize=(10, 8), dpi=3600)
...
ax_main.set_xticklabels([f'{x}' for x in x_ticks], fontsize=30)


# 自定义Y轴刻度，压缩10^0到10^2之间的间隔
y_ticks = [200, 400, 600, 800, 1000]  # 原刻度
y_labels = ['$200$', '$400$', '$600$', '$800$', '$1000$']
plt.yticks(y_ticks, y_labels, fontsize=30)
plt.ylim(0, 1000)

# 强制设置刻度线朝向内侧
ax_main.tick_params(axis='both', which='both', direction='in')
ax_main.tick_params(axis='x', which='major', bottom=True, top=True, length=8, width=1.5)
ax_main.tick_params(axis='y', which='major', left=True, right=True, length=8, width=1.5)
ax_main.tick_params(axis='x', which='minor', bottom=True, top=True, length=4, width=1)
ax_main.tick_params(axis='y', which='minor', left=True, right=True, length=4, width=1)
...
# 在主图绘制完成后，添加插图
ax_inset = plt.axes([0.5, 0.26, 0.35, 0.25])  # [x, y, width, height]
...
# 设置插图主要刻度线
ax_inset.tick_params(axis='x', which='major', bottom=True, top=True, length=4, width=0.5, labelsize=10)
ax_inset.tick_params(axis='y', which='major', left=True, right=True, length=4, width=0.5, labelsize=10)
ax_inset.tick_params(axis='x', which='minor', bottom=True, top=True, length=2, width=0.5)
...
ax_inset.set_xticklabels([f'{x}' for x in inset_x_ticks], fontsize=24)

# 设置插图y轴刻度
inset_y_ticks = [1, 2, 3, 4]# 根据您的范围调整
ax_inset.set_yticks(inset_y_ticks)
ax_inset.set_yticklabels([f'{y}' for y in inset_y_ticks], fontsize=24)
...
# 为插图添加y轴标签，明确单位
ax_inset.set_ylabel('ms', fontsize=24)
...
# 保存文件
plt.savefig("pic/w1-server/Wiki_server_computation_time_comparison2.pdf", dpi=3600, bbox_inches='tight', format='pdf')
# plt.savefig("pic/w1-server/Enron_server_computation_time_comparison.svg", dpi=3600, bbox_inches='tight', format='svg')
# plt.savefig("pic/w1-server/Enron_server_computation_time_comparison.png", dpi=3600, bbox_inches='tight', format='png')
