...
# 预设文件地址（实际使用时替换）    
file_paths = {
    "FDXT": "cmd/HDXT/configs/Wiki_USENIX_filecnt_sorted_storage_fdxt_1.csv",
    "ODXT": "cmd/HDXT/configs/Wiki_USENIX_filecnt_sorted_storage_1.csv",
    "SDSSE-CQ": "cmd/HDXT/configs/Wiki_USENIX_filecnt_sorted_storage_sdssecq_1.csv"
}
...
for scheme, file_path in file_paths.items():
    try:
        ...
        # 选择每隔1000个点
        indices_below_10000 = np.arange(500, len(x_values_below_10000), 500)
        ...
        # 选择每隔5000个点
        indices_above_10000 = np.arange(500, len(x_values_above_10000), 500)
...
# 保存文件
plt.savefig("pic/storage/Wiki_storage_comparison_new.pdf", dpi=3600, format='pdf')
