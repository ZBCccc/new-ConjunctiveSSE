...
for scheme, file_path in file_paths.items():
    try:
        ...
        # 合并采样结果
        x_sampled = pd.concat([x_sampled_below_10000, x_sampled_above_10000])
        y_sampled = pd.concat([y_sampled_below_10000, y_sampled_above_10000])
        # 合并采样结果，确保最后一个点被包含
        x_sampled = pd.concat([x_sampled, last_point_above])
        y_sampled = pd.concat([y_sampled, y_values_above_10000.iloc[[-1]]])
        # 绘制主图线并获取线条对象
...
# 设置对数坐标轴的刻度
y_ticks = [0.1, 1, 10, 100, 1000, 10000, 100000, 1000000]  # 原刻度
y_labels = ['$10^{-1}$', '$10^{0}$', '$10^{1}$', '$10^{2}$', '$10^{3}$', '$10^{4}$', '$10^{5}$', '$10^{6}$']
plt.yticks(y_ticks, y_labels, fontsize=50)
plt.ylim(0.1, 1000000)  # 设置y轴范围为0.01到10^5
...
# ax_main.yaxis.set_minor_locator(plt.MultipleLocator(500))  # 调整次要刻度间隔
...
# 保存文件
plt.savefig("pic/storage/Crime_storage_comparison_new.pdf", dpi=3600, format='pdf')
