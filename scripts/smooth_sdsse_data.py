...
def smooth_data(input_file):
    """
    对CSV文件中的数据进行平滑处理:
    - 如果clientTime > 175,将其置为附近的第一个小于175的值
    - 如果serverTime > 90000,将其置为附近的第一个小于90000的值
    ...
    for i in range(len(df)):
        if df.at[i, 'clientTime'] > 175:
            # 寻找附近的第一个小于175的值
            found = False
            # 先向前查找
            for j in range(i-1, -1, -1):
                if df.at[j, 'clientTime'] <= 175:
    ...
    # 平滑处理serverTime列
    for i in range(len(df)):
        if df.at[i, 'serverTime'] > 90000:
            # 寻找附近的第一个小于90000的值
            found = False
            # 先向前查找
            for j in range(i-1, -1, -1):
                if df.at[j, 'serverTime'] <= 90000:
...
def main():
    input_file = "result/Search/SDSSE-CQ/Wiki_USENIX/w1_keywords_2.csv"
    
    if not os.path.exists(input_file):
        print(f"错误: 文件 '{input_file}' 不存在")
        sys.exit(1)
    
    smooth_data(input_file)
