...
def smooth_data(input_file):
    """
    对CSV文件中的数据进行平滑处理:
    - 如果clientTime > 47,将其置为附近的第一个小于47的值
    - 如果serverTime > 45,将其置为附近的第一个小于45的值
    ...
    for i in range(len(df)):
        if df.at[i, 'clientTime'] > 47:
            # 寻找附近的第一个小于47的值
            found = False
            # 先向前查找
            for j in range(i-1, -1, -1):
                if df.at[j, 'clientTime'] <= 47:
    ...
    # 平滑处理serverTime列
    for i in range(len(df)):
        if df.at[i, 'serverTime'] > 45:
            # 寻找附近的第一个小于45的值
            found = False
            # 先向前查找
            for j in range(i-1, -1, -1):
                if df.at[j, 'serverTime'] <= 45:
...
def main():
    input_file = "result/Search/FDXT/Wiki_USENIX/w1_keywords_2.csv"
