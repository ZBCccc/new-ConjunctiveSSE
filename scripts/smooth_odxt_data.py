...
def smooth_data(input_file):
    """
    对CSV文件中的数据进行平滑处理：
    - 如果clientTime > 60，将其置为附近的第一个小于60的值
    - 如果serverTime > 75，将其置为附近的第一个小于75的值
    ...
    for i in range(len(df)):
        if df.at[i, 'clientTime'] > 60:
            # 寻找附近的第一个小于60的值
            found = False
            # 先向前查找
            for j in range(i-1, -1, -1):
                if df.at[j, 'clientTime'] <= 60:
    ...
    # 平滑处理serverTime列
    for i in range(len(df)):
        if df.at[i, 'serverTime'] > 75:
            # 寻找附近的第一个小于75的值
            found = False
            # 先向前查找
            for j in range(i-1, -1, -1):
                if df.at[j, 'serverTime'] <= 75:
...
def main():
    input_file = "result/Search/ODXT/Crime_USENIX_REV/2025-03-15_20-43-23.csv"
