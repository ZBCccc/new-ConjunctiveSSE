...
def calculate_storage(json_file_path):
    ...
    try:
        ...
        # 计算总空间占用
        csv_file_path = json_file_path.replace('.json', '_storage_sdssecq_1.csv')
        with open(csv_file_path, 'w', encoding='utf-8') as csv_file:
            ...
            for i, (keyword, file_count) in enumerate(data.items()):
                ...
                bf_storage = d_max * 0.0024 * 1024 * 8 * 2
                storage = (i + 1) * calculate_binary_storage(file_count) * 3 + bf_storage + total_bf
                total_bf += bf_storage
                csv_file.write(f"{i + 1},{total_counter},{storage}\n")
