...
file_paths = {
    "FDXT": "scripts/search-tokens/data/w2_fdxt_crime_storage_data.csv",
    "ODXT": "scripts/search-tokens/data/w2_odxt_crime_storage_data.csv",
    "SDSSE-CQ": "scripts/search-tokens/data/w2_sdssecq_crime_storage_data.csv"
}
...
for scheme, file_path in file_paths.items():
    try:
        ...
        # Use sampled 'updtw1' column for x-axis
        x_values = sampled_df['updtw1']
        
        # Use sampled 'storage' column divided by 1024 for y-axis
        y_values = sampled_df['storage'] / 1024.0
...
# 创建带有断点的y轴
ax_main.set_ylim(0, 1500)
ax_main.set_yticks([300, 600, 900, 1200, 1500])
ax_main.set_yticklabels(['300', '600', '900', '1200', '1500'], fontsize=50)
...
ax_main.yaxis.set_minor_locator(plt.MultipleLocator(150))  # 调整次要刻度间隔
...
# 保存文件
plt.savefig("scripts/search-tokens/pic/Crime_w2_comm_cost_comparison.pdf", dpi=3600, format='pdf')
